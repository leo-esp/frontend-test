...
describe('Testing actions creators', () => {
    ...
    it('should create an action to list movies', () => {
        ...
        expect(listMovies(movies)).toEqual(expectedAction)
    })

    it('should create an action to list favorite movies', () => {
        ...
        expect(showFavorites(favoriteMovies)).toEqual(expectedAction)
    })

    it('should create an action to error message', () => {
        const msg = "Sorry! We couldn't find any movies directed by that director!";
        const expectedAction = {
            type: 'ERROR',
            msg
        }
        expect(error(msg)).toEqual(expectedAction)
    })

    it('should create an action to favorite movie', () => {
        const movie = { "unit": 6226, "show_id": 70299043, "show_title": "Attack on Titan", "release_year": "2013", "rating": "4.6", "category": "Anime", "show_cast": "Yuki Kaji, Yui Ishikawa, Marina Inoue, Daisuke Ono, Hiro Shimono, Hiroshi Kamiya, Keiji Fujiwara, Kish\u00f4 Taniyama, Romi Park, Ryota Ohsaka", "director": "", "summary": "For over a century, people have been living behind barricades to block out the giant Titans that threaten to destroy the human race. When a Titan destroys his hometown, young Eren Yeager becomes determined to fight back.", "poster": "http:\/\/cdn-2.nflximg.com\/en_us\/boxshots\/ghd\/70299043.jpg", "mediatype": 1 }
        const expectedAction = {
            type: 'FAVORITE',
            movie
        }
        expect(favorite(movie)).toEqual(expectedAction)
    })
})